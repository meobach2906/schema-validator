...
describe('_validate', () => {
  ...
  it('_validate.validate()', () => {
    doTest = (test) => {
      const result = _validate.validate(test.input);
    ...
    const tests = [
      {
        input: {
          schema: [{
            ...
            date_convert_to_iso_string_then_lowercase: { type: 'date', require: true, nullable: false, to: `iso_datetime,to_lower` },
            strict_object: {
              string: { type: 'string' },
            },
            set: { type: 'array', element: { type: 'string' }, check: { set: true } },
            unique_code: { type: 'array', check: { unique: 'code,age' }, element: { code: { type: 'string' }, age: { type: 'number' } } },
        ...
        expect: {
          ...
          errors: [
            ...
            {
              invalid: "unique",
              field: "0.unique_code.0.code,age"
            },
            {
              invalid: "unique",
              field: "0.unique_code.3.code,age"
            },
            {
